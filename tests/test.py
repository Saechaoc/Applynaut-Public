...
from config.utils import prGreen, prRed, prYellow
...
def checkPython():
    try:
        if (sys.version):
            ...
            prRed(
                "❌ Python is not installed please install Python first: https://www.python.org/downloads/")
...
def checkSelenium():
    try:
        ...
        prRed(
            "❌ Selenium not present. Install Selenium: https://pypi.org/project/selenium/")
...
def checkFirefox():
    try:
        ...
        if (output):
            ...
            prRed(
                "❌ Firefox not present. Install firefox: https://www.mozilla.org/en-US/firefox/")
...
def checkSeleniumLinkedin():
    ...
    # chrome needs to support same version as users installed version
    browser = webdriver.Chrome(
        executable_path=r"C:\Users\chris\Documents\GitHub\EasyApplyJobsBot\chromedriver-win64\chromedriver.exe")
    ...
    try:
        ...
        if "Feed" in browser.title:
            prGreen(
                '✅ Successfully you are logged in to Linkedin, you can now run main bot script!')
        else:
            prRed(
                '❌ You are not automatically logged in, please set up your Firefox Account correctly.')
    except Exception as e:
        prRed(e)
    finally:
        browser.quit()
